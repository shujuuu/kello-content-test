      ...
      *DESCRIÇÃO: Conversão GALHOCOD060
       ...
       FILE-CONTROL.
           ...
           SELECT  COD050A ASSIGN        TO              PATH-COD050A
                           ...
                           ALTERNATE RECORD KEY                    IS
                                CHAVE-ENVIO-CO50A =  DATA-ENVIO-CO50A
                                                    NR-CONTRATO-CO50A
                                                           ITEM-CO50A.

           COPY COPX051.

           SELECT  COD051A ASSIGN       TO            PATH-COD051A
                           ORGANIZATION IS                 INDEXED
                           ACCESS MODE  IS                 DYNAMIC
                           STATUS       IS              ST-COD051A
                           LOCK MODE IS MANUAL WITH LOCK ON RECORD
                           RECORD KEY   IS           CHAVE-CO51A =
                                                 NR-CONTRATO-CO51A
                                                        ITEM-CO51A
                           ALTERNATE RECORD KEY
                                        IS       DATA-VENCTO-CO51A
                                        WITH            DUPLICATES
                           ALTERNATE RECORD KEY IS    ALT1-CO51A =
                                                 NR-CONTRATO-CO51A
                                                   REALIZADO-CO51A
                                                 DATA-VENCTO-CO51A
                                                        ITEM-CO51A.
      ...
      *    campo DIAS-PRAZO-CO50.
           ...
           05  DATA-ENVIO-CO50A     PIC 9(8).
           05  OBSERVACAO-CO50A     PIC X(300).
           05  FILLER               PIC X(100).
      ...
      **** SER ALTERADO E A DATAPREVISTA P/ A DATA DE PAGTO
       ...
       FD  COD051A.
       01  REG-COD051A.
           05  NR-CONTRATO-CO51A    PIC 9(04).
           05  ITEM-CO51A           PIC 99.
           05  CODBRINDE-CO51A      PIC 999.
           05  CURSO-CO51A          PIC 9(3).
           05  TURMA-CO51A          PIC XX.
           05  QTDE-POR-FORM-CO51A  PIC 9(5).
      *    qtde-por-form = qtde-brindes ou qtde por formando
      *    quando for qtde-brindes o campo qtde-form-CO51 deverá ficar
      *    zerado
           05  QTDE-FORM-CO51A      PIC 9(4).
           05  CUSTO-UNIT-CO51A     PIC 9(8)V99 COMP-3.
      *    se custo-unit-CO51 = zeros o custo a ser considerado será
      *    do cadastro de brinde
           05  VALOR-PREVISTO-CO51A PIC 9(8)V99 COMP-3.
           05  DATA-VENCTO-CO51A    PIC 9(8).
      *    DATA-VENCTO - AAAA/MM/DD.
           05  DATA-SOLICIT-CO51A   PIC 9(8)    COMP-3.
           05  SUSP-PREV-DEF-CO51A  PIC 9.
      *    0-PREVISTO   1-DEFINITIVO  2-SUSPENSO
           05  VALOR-PAGO-CO51A     PIC 9(8)V99 COMP-3.
           05  DATA-PAGTO-CO51A     PIC 9(8)    COMP-3.
           05  REALIZADO-CO51A      PIC 9.
      *    0-NAO  1-SIM
           05  DIAS-PRAZO-CO51A     PIC 9(4).
      *    Prazo médio até a data prevista de venda (do contrato)
      *    Se DATA-PAGTO-CO51 = zeros
      *       dias entre DATA-VENCTO-CO51 até DATA-PREV-VENDA-CO40
      *    Senão dias entre DATA-PAGTO-CO51 até DATA-PREV-VENDA-CO40.
      *    Qualquer alteração efetuada nos campos data-pagto-CO51
      *    data-vencto-CO51 e data-prev-venda-co40 deve-se alterar o
      *    campo DIAS-PRAZO-CO51.
           05  COD-FORNEC-CO51A     PIC 9(6)    COMP-3.
      ...
      **** SER ALTERADO E A DATAPREVISTA P/ A DATA DE PAGTO
       ...
       01  VARIAVEIS.
           ...
           05  ST-COD051A            PIC XX       VALUE SPACES.
           05  EMP-REFERENCIA.
               10  FILLER            PIC X(15)
       ...
       renomear-arquivos section.
           ...
           MOVE CODIGO-CA001           TO EMP-REC
           MOVE "COD051"               TO ARQ-REC
           MOVE EMPRESA-REF            TO PATH-COD051A

           OPEN I-O COD050A COD051A
           CLOSE    COD050A COD051A
           ...
           MOVE CODIGO-CA001           TO EMP-REC
           MOVE "COD051"               TO ARQ-REC
           MOVE EMPRESA-REF            TO PATH-COD051

           MOVE CODIGO-CA001           TO EMP-REC
           MOVE "COD051B"              TO ARQ-REC
           MOVE EMPRESA-REF            TO PATH-COD051A

           call "CBL_RENAME_FILE" using PATH-COD051
                                        PATH-COD051A
                              returning status-code

           STRING PATH-COD051 ".idx" DELIMITED BY " " INTO PATH-COD051

           STRING PATH-COD051A ".idx" DELIMITED BY " " INTO PATH-COD051A
       ...
       converter-arquivo section.
           ...
           PERFORM UNTIL ST-COD050A = "10"
                READ COD050A NEXT AT END
                     MOVE "10" TO ST-COD050A
                NOT AT END
                     DISPLAY "REG-COD050B = " REG-COD050A
                     ...
                     MOVE DATA-ENVIO-CO50A      TO DATA-ENVIO-CO50
                     MOVE OBSERVACAO-CO50A      TO OBSERVACAO-CO50
           ...
           INITIALIZE REG-COD051A
           START COD051A KEY IS NOT LESS CHAVE-CO51A INVALID KEY
                MOVE "10" TO ST-COD051A.

           PERFORM UNTIL ST-COD051A = "10"
                READ COD051A NEXT AT END
                     MOVE "10" TO ST-COD051A
                NOT AT END
                     DISPLAY "REG-COD051B = " REG-COD051A

                     MOVE NR-CONTRATO-CO51A     TO NR-CONTRATO-CO51
                     MOVE ITEM-CO51A            TO ITEM-CO51
                     MOVE CODBRINDE-CO51A       TO CODBRINDE-CO51
                     MOVE CURSO-CO51A           TO CURSO-CO51
                     MOVE TURMA-CO51A           TO TURMA-CO51
                     MOVE QTDE-POR-FORM-CO51A   TO QTDE-POR-FORM-CO51
                     MOVE QTDE-FORM-CO51A       TO QTDE-FORM-CO51
                     MOVE CUSTO-UNIT-CO51A      TO CUSTO-UNIT-CO51
                     MOVE VALOR-PREVISTO-CO51A  TO VALOR-PREVISTO-CO51
                     MOVE DATA-VENCTO-CO51A     TO DATA-VENCTO-CO51
                     MOVE DATA-SOLICIT-CO51A    TO DATA-SOLICIT-CO51
                     MOVE SUSP-PREV-DEF-CO51A   TO SUSP-PREV-DEF-CO51
                     MOVE VALOR-PAGO-CO51A      TO VALOR-PAGO-CO51
                     MOVE DATA-PAGTO-CO51A      TO DATA-PAGTO-CO51
                     MOVE REALIZADO-CO51A       TO REALIZADO-CO51
                     MOVE DIAS-PRAZO-CO51A      TO DIAS-PRAZO-CO51
                     MOVE COD-FORNEC-CO51A      TO COD-FORNEC-CO51

                     WRITE REG-COD051 INVALID KEY
                           MOVE "Erro de Gravação...COD051" TO MENSAGEM
                           MOVE "C" TO TIPO-MSG
                           PERFORM EXIBIR-MENSAGEM
                     END-WRITE
       ...
       fechar-arquivos section.
           close cod050 cod050a cod051 cod051a.
