      ...
      *FUNÇÃO: REPORTAGEM POR EQUIPE - PERÍODO - CLASSIF- ABC
      ...
      *        dentro do intervalo de reportagem solicitado.
       ...
       FILE-CONTROL.
           ...
           SELECT WORK ASSIGN TO VARIA-W
                  ...
                  ALTERNATE RECORD KEY IS CLASSIFICACAO-WK
                                           WITH DUPLICATES
      ...
      *    variaveis p/ calculo da curva abc
           05  VALOR-ACUM            PIC 9(8)V99  VALUE ZEROS.
           05  VALOR-A               PIC 9(8)V99  VALUE ZEROS.
           05  VALOR-B               PIC 9(8)V99  VALUE ZEROS.
           05  VALOR-C               PIC 9(8)V99  VALUE ZEROS.
           05  NIVEL-CURVA-ABC       PIC 9        VALUE ZEROS.
      ...
      -    "  QT.REPORTAGEM   VLR-REPORTAGEM CLAS".
       ...
       01  CAB05.
           ...
           "QTDE-FUNCION.          PRODUCAO    QTDE-REPORT   VLR-TOTAL-R
      -    "EPORT".
       01  LINTOT.
      ...
      *    MOVE 1 TO COD-USUARIO-W.
       ...
       CALCULO-CURVA-ABC SECTION.
           COMPUTE VALOR-A = TOT-VLR-REPORT * 0,60.
           COMPUTE VALOR-B = TOT-VLR-REPORT * 0,30.
           COMPUTE VALOR-C = TOT-VLR-REPORT * 0,10.
           MOVE ZEROS TO VALOR-ACUM
           MOVE 1     TO NIVEL-CURVA-ABC.

           MOVE ZEROS TO VLR-REPORT-WK.
           START WORK KEY IS NOT < VLR-REPORT-WK INVALID KEY
              MOVE "10" TO ST-WORK.
           PERFORM UNTIL ST-WORK  = "10"
             READ WORK NEXT RECORD AT END MOVE "10" TO ST-WORK
               NOT AT END
                 EVALUATE NIVEL-CURVA-ABC
                   WHEN 1 MOVE "A"            TO CLASSIFICACAO-WK
                          ADD VLR-REPORT-WK   TO VALOR-ACUM
                          IF VALOR-ACUM > VALOR-A
                             COMPUTE VALOR-ACUM = VALOR-ACUM - VALOR-A
                             MOVE 2               TO NIVEL-CURVA-ABC
                          REWRITE REG-WORK
                          END-REWRITE
                   WHEN 2 MOVE "B"            TO CLASSIFICACAO-WK
                          ADD VLR-REPORT-WK   TO VALOR-ACUM
                          IF VALOR-ACUM > VALOR-B
                             COMPUTE VALOR-ACUM = VALOR-ACUM - VALOR-B
                             MOVE 3               TO NIVEL-CURVA-ABC
                          REWRITE REG-WORK
                          END-REWRITE
                   WHEN 3 MOVE "C"            TO CLASSIFICACAO-WK
                          REWRITE REG-WORK
                          END-REWRITE
                 END-EVALUATE
                 REWRITE REG-WORK
           ...
           EVALUATE GS-ORDEM
             ...
             WHEN 3
                ...
                START WORK KEY IS NOT < VLR-REPORT-WK INVALID KEY
                      MOVE "10" TO ST-WORK
             WHEN 4
                MOVE "CLASSIFICACAO" TO GS-DESCR-ORDEM
                START WORK KEY IS NOT < CLASSIFICACAO-WK INVALID KEY
                      MOVE "10" TO ST-WORK
           END-EVALUATE.
       TOTALIZA SECTION.
       ...
       MOVER-DADOS-RELATORIO SECTION.
           ...
           MOVE QT-FILM-FIT-WK    TO QTDE-E
           MOVE QTDE-E            TO LINDET-REL(48: 11)
           MOVE QT-REPORT-WK      TO QTDE-E1
           MOVE QTDE-E            TO LINDET-REL(63: 15)
           MOVE VLR-REPORT-WK     TO VALOR-E
           MOVE VALOR-E           TO LINDET-REL(82: 14)
           MOVE CLASSIFICACAO-WK  TO LINDET-REL(94: 2).
       ...
       TOTALIZA-REL SECTION.
           ...
           MOVE TOT-REPORT        TO QTDE-E1
           MOVE QTDE-E1           TO LINTOT-REL(34: 13)
           MOVE TOT-VLR-REPORT    TO VALOR-E
           MOVE VALOR-E           TO LINTOT-REL(55: 13)
